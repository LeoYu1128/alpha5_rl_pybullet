    ...
    while not done and episode_length < max_steps:
        ...
        if should_capture:
            ...
            try:
                ...
                # 转换为PIL图像 - 正确处理返回值
                rgb_data = img_arr[2]  # 第3个元素是RGB像素数据
    ...
    while not done and episode_length < max_steps:
        ...
        if should_capture:
            ...
            try:
                ...
                # 转换为PIL图像
                # 正确处理返回的图像数据
                rgb_data = img_arr[2] if len(img_arr) >= 3 else img_arr
                rgb_array = np.array(rgb_data, dtype=np.uint8)
                if rgb_array.ndim == 1:
                    rgb_array = rgb_array.reshape((height, width, 4))
                rgb_array = rgb_array[:, :, :3]  # 去掉alpha通道
...
def train_alpha_reach(
    ...
    # 初始化 W&B
    wandb.init(
        project="alpha5-robotic-arm",           # 项目名称（会在 W&B 网站上显示）
        name=exp_name,                          # 实验名称（使用你的时间戳名称）
        config={                                # 记录超参数
            "algorithm": algorithm,
            "total_timesteps": total_timesteps,
            "num_envs": num_envs,
        },
        sync_tensorboard=True,                  # 自动同步 TensorBoard 日志
        save_code=True,                         # 保存代码快照
        dir=exp_dir                             # W&B 文件保存位置
    )
    # ========== 添加结束 ⬆️ ==========
    ...
    env=train_env, 
    tensorboard_log=os.path.join(log_path, "tensorboard"),  # 添加这行
    **configs[algorithm]
    )
    ...
    wandb_callback = WandbCallback(
        model_save_path=os.path.join(save_path, f"wandb_{algorithm}"),
        verbose=2,
        gradient_save_freq=1000,  # 每1000步保存一次梯度信息
    )
    model.learn(
        total_timesteps=total_timesteps,
        callback=[eval_callback, checkpoint_callback, progress_callback,wandb_callback],
        progress_bar=True
    )
    ...
    # 关闭 W&B
    wandb.finish()
    print(f"📊 W&B 日志已上传: https://wandb.ai/{wandb.run.entity}/{wandb.run.project}/runs/{wandb.run.id}")
