...
class AlphaReachEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, max_steps=500, reward_type='dense',
             enable_target_drift=False, enable_domain_randomization=False, 
        ...
        self.target_actual_mass = 1.36           # 实际机械臂质量 (kg)
        self.target_underwater_mass = 0.9        # 水下有效质量 (kg)
        ...
        print(f"[物理参数] 质量缩放: {self.robot_mass_scale:.4f}, 浮力补偿: {self.buoyancy_compensation_ratio:.4f}")
        # ==========================================
        
        # 水流参数
        self.current_velocity = np.array([0.1, 0.05, 0.0])  # 水流速度 m/s
        self.current_variation = True  # 水流是否变化
        self.turbulence_strength = 0.1  # 湍流强度
        # ========== 新增: 传感器噪声参数 ==========
        self.enable_sensor_noise = True              # 是否启用噪声
        self.position_noise_std = 0.002              # 关节位置噪声标准差 (约0.2%)
        self.velocity_noise_std = 0.008              # 关节速度噪声标准差 (约0.8%)
        self.ee_position_noise_std = 0.003           # 末端执行器位置噪声标准差 (0.3cm)
        print(f"[传感器噪声] 位置: {self.position_noise_std}, 速度: {self.velocity_noise_std}, 末端: {self.ee_position_noise_std}")
        # ==========================================
    ...
    def _setup_underwater_dynamics(self):
        ...
        # 验证质量缩放是否正确
        print(f"[质量缩放] URDF总质量: {total_urdf_mass:.4f} kg")
        print(f"[质量缩放] 缩放后总质量: {total_scaled_mass:.4f} kg (目标: {self.target_actual_mass:.4f} kg)")
        print(f"[质量缩放] 有质量的连杆数量: {len(self.link_indices)}")
    ...
    def _get_joint_positions(self):
        ...
        for joint_idx in self.main_joint_indices:
            ...
            true_position = joint_state[0]  # 真实位置
    ...
    def _get_joint_velocities(self):
        ...
        for joint_idx in self.main_joint_indices:
            ...
            true_velocity = joint_state[1]  # 真实速度
    ...
    def _apply_action(self, action):
        ...
        if current_distance > 0.15:
            scale = 0.15
        elif current_distance > 0.08:  # 注意這裡改成 > 而不是 < 和 >
            scale = 0.06
        elif current_distance > 0.05:  # current_distance 介於0.04和0.08之間
            scale = 0.03
        else:
            scale = 0.01
    ...
    def step(self, action):
        ...
        return observation, reward, terminated, truncated, info  # 分开返回 terminated 和 truncated
