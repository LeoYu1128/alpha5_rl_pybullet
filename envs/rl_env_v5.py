...
class UnderwaterAlphaReachEnv(gym.Env):
    """
    水下Alpha机械臂到达任务环境
    ...
    def _sample_target_position(self):
        """在水下工作空间内均勻采樣目標位置（安全範圍內）"""
        max_attempts = 100
        safe_radius_min = 0.15       # 避免太靠近基座
        safe_radius_max = 0.35       # 避免靠近最大可達半徑
        z_min = 0.12                 # 安全水下高度下限
        z_max = 0.38                 # 安全水下高度上限
        ...
        # 如果多次嘗試未成功，返回預設安全位置
        return np.array([0.2, 0.0, 0.2], dtype=np.float32)
    ...
    def _compute_reward(self):
        ...
        # 基于距离的密集奖励
        distance_reward = 2.0 / (1.0 + distance * 15)  # 水下精度要求更高
        
        # 成功奖励
        success_threshold = 0.08  # 水下阈值稍大一些，因为控制更困难
        success_reward = 15.0 if distance < success_threshold else 0.0
        ...
        # 能耗惩罚 (水下操作耗能更大)
        energy_penalty = -0.005 * np.sum(np.abs(joint_velocities))
        ...
        # 如果末端执行器速度与水流速度接近，给予小奖励
        velocity_diff = np.linalg.norm(ee_velocity - self.current_velocity_actual)
        current_adaptation_reward = 0.1 / (1.0 + velocity_diff)
        
        # 工作空间约束惩罚
        workspace_penalty = 0.0
        if distance > self.workspace_radius:
            workspace_penalty = -2.0  # 水下惩罚更重
        
        total_reward = (distance_reward + success_reward + smoothness_penalty + 
                       energy_penalty + current_adaptation_reward + workspace_penalty)
        
        # 确保返回Python float类型
        total_reward = float(total_reward)
        success = bool(distance < success_threshold)
    ...
    def reset(self, seed=None, options=None):
        ...
        # 计算初始距离
        ee_pos = self._get_end_effector_position()
        initial_distance = float(np.linalg.norm(ee_pos - self.target_position))
...
if __name__ == "__main__":
    # 创建水下环境
    env = UnderwaterAlphaReachEnv(render_mode="human")
    ...
    print(f"水流速度: {info['current_velocity']}")
